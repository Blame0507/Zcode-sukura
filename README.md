...
**ZCode Sakura** 是基于开源 [ZCode](https://github.com/zai-org/zcode)(Apache-2.0)的个人定制桌面版分支:樱花主题开箱即用、与官方版数据完全隔离、更新通道自主可控、技能全部来自社区开源实现。本分支只保留桌面版形态,专注做好一件事。

> 非官方分支,与 Z.ai 无隶属或被背书关系。上游的社区与支持渠道请访问上游仓库。

## 这个分支做了什么

- **樱花主题(默认主题)**:粉白主题色系、对话区飘落樱花动画(纯合成器动画,低功耗)、壁纸层与渐变融合、主题切换过渡动画
- **品牌视觉**:粉底白字 logo 与全套应用图标(安装器/托盘/运行时)
- **数据隔离**:数据根固定在 `~/.zcode-sakura-home/.zcode`,与官方闭源版并排安装、双开互不干扰,不互抢锁、不串任务与设置
...
- 壁纸素材、粉色 logo/图标中的图形元素在公开再分发前需自行确认权利或替换;发布前的完整清理清单建议包含:替换壁纸素材、清洗其 git 历史、评估产品改名
